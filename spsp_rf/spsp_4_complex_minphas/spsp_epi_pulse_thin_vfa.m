...
% Target Fs ~520-580 to put opposed nulls in benign (or at least not lac/pyr)
% regions. Potential problem is the pyruvate-hydrate is in the opposed null when pyruvate is excited.  Expect 10:1 ratio should minimize any artifacts. 
% Current choice is pulse#3

% JWG moved the fctr to 300 and updated gmax and smax
% Choose #8 for truest of the true nulls
...
z_thk = 0.4; %shoot for 3mm min slice
z_tb = 3.; ss_type = 'EP Whole';

ptype = 'ex';
z_ftype='ls';
z_d1 = 0.01;
z_d2 = 0.01;
s_ftype = 'min' ; %spectral filter type: 'min' (default), 'max', 'lin'
...
[f,z,m] = ss_plot(g, rf, SS_TS, ptype, z_thk*2, 2*[min(fspec) max(fspec)], SS_GAMMA, fplot);
